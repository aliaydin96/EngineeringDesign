...
for i in range(30):
  ...
  if(data[(rmin_ind-i)] < 240):
    min_ind = rmin_ind-i
sel_r = data[min_ind:(max_ind)]
sel_th = theta[min_ind:(max_ind)]
...
if((a1 >= 130)&(de1 > 400)):
  if(a2 < 80):
    ...
    x_corner1 = x_in + x_corner1*np.cos(heading+pos_flag*math.pi/2) - y_corner1*np.sin(heading+pos_flag*math.pi/2)
    y_corner1 = y_in + x_corner1*np.sin(heading+pos_flag*math.pi/2) + y_corner1*np.cos(heading+pos_flag*math.pi/2)
    x_corner2 = x_in + x_corner2*np.cos(heading+pos_flag*math.pi/2) - y_corner2*np.sin(heading+pos_flag*math.pi/2)
    y_corner2 = y_in + x_corner2*np.sin(heading+pos_flag*math.pi/2) + y_corner2*np.cos(heading+pos_flag*math.pi/2)
