...
title= dataName+ '(Purely Measurement) with Self Position of Robot'
plt.title(title)
plt.savefig('C:\\Users\\enes_\\Desktop\\Coni\\Images\\'+title)
...
title= dataName+ '(Cleaning with average filter)'
plt.title(title)
plt.savefig('C:\\Users\\enes_\\Desktop\\Coni\\Images\\'+title)
...
plt.xlabel('X')
plt.ylabel('Y')
title= dataName+ 'Decision of Destination Point'
plt.title(title)
plt.savefig('C:\\Users\\enes_\\Desktop\\Coni\\Images\\'+title)
...
for i in range(sortedRho.shape[0]-1):
    ...
    for j in range(ADPM.shape[1]):
        ...
        if -pi/2<thetaDif<pi/2 and ADPM[0][j]*abs(np.sin(thetaDif))<100 and ADPM[0][j] < ADPM[0][maxIndex]-200 and k==0:
            k=1;
    if k==0:
        maxIndice=maxIndex
        break 
...
plt.xlabel('X')
plt.ylabel('Y')
title=dataName+'Final Destination Point'
